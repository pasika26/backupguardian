...
async function testDatabaseSchema() {
  try {
    ...
    const backupCount = await query('SELECT COUNT(*) as count FROM backups');
    const testRunCount = await query('SELECT COUNT(*) as count FROM test_runs');
    const testResultCount = await query('SELECT COUNT(*) as count FROM test_results');
    
    console.log(`Users: ${userCount.rows[0].count}`);
    console.log(`Backups: ${backupCount.rows[0].count}`);
    console.log(`Test Runs: ${testRunCount.rows[0].count}`);
    console.log(`Test Results: ${testResultCount.rows[0].count}`);
    
    // Test 3: Check foreign key relationships
    console.log('\n3. Testing foreign key relationships...');
    
    // Test user -> backups relationship
    const userBackups = await query(`
    ...
    const backupTestRuns = await query(`
    ...
    const testRunResults = await query(`
      SELECT 
        tr.id,
    ...
    testRunResults.rows.forEach(row => {
      ...
      console.log(`  Run ${shortId}... (${row.run_status}): ${row.result_count} results`);
    });
    ...
    const orphanedBackups = await query(`
      SELECT COUNT(*) as count FROM backups b
      WHERE NOT EXISTS (SELECT 1 FROM users u WHERE u.id = b.user_id);
    `);
    
    const orphanedTestRuns = await query(`
    ...
    const orphanedTestResults = await query(`
      SELECT COUNT(*) as count FROM test_results tres
    ...
    return {
      ...
      rowCounts: {
        ...
        backups: backupCount.rows[0].count,
        testRuns: testRunCount.rows[0].count,
        testResults: testResultCount.rows[0].count
      },
      relationshipsWork: true,
      dataIntegrity: {
        orphanedBackups: orphanedBackups.rows[0].count,
        orphanedTestRuns: orphanedTestRuns.rows[0].count,
        orphanedTestResults: orphanedTestResults.rows[0].count
      }
    };
